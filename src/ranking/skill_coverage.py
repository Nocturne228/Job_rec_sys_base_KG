...
class SkillCoverageCalculator:
    ...
    def __init__(
        self, skill_level_mapping: Optional[Dict[str, int]] = None, gat_weighter=None
    ):
        """
        Args:
    ...
    def calculate_coverage(
        ...
        if total_skills == 0:
            ...
            coverage_score = (
                required_weight * required_metrics["coverage_ratio"]
    ...
    def _gat_weighted_coverage(
        self, user_skills: Dict[str, str], job_required_skills: Dict[str, str]
    ) -> float:
        """
        ...
        for skill_id, required_level in job_required_skills.items():
            ...
            user_level_num = self.skill_level_mapping.get(
                str(getattr(user_level, "value", user_level)).lower(), 0
            )
            required_level_num = self.skill_level_mapping.get(
                str(getattr(required_level, "value", required_level)).lower(), 0
            )
